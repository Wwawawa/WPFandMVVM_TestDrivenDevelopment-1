...
namespace FriendStroage.UITests.ViewModel
{
    public class FriendEditViewModelTest
    {
        ...
        [Fact]
        public void ShouldPublishSavedEventWhenSaveCommandIsExecuted()
        {
            _viewModel.Load(_friendId);
            _viewModel.Friend.FirstName = "Changed";
            ...
            MessageDialogResult result, int expectedDeleteFriendCalls)
        {
            ...
            _messageDialogServiceMock.Setup(ds => ds.ShowYesNoDialog(
                It.IsAny<string>(), It.IsAny<string>())).Returns(result);

            _viewModel.DeleteCommand.Execute(null);

            _dataProviderMock.Verify(
                dp => dp.DeleteFriend(_friendId),
                Times.Exactly(expectedDeleteFriendCalls));
            _messageDialogServiceMock.Verify(ds => ds.ShowYesNoDialog(
